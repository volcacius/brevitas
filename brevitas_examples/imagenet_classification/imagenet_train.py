...
from brevitas.onnx import export_dpuv1_onnx, export_dpuv2_onnx
...
parser.add_argument('--export', default='', type=str,
                    help='Export model to DPUv1 or DPUv2')
...
def export_onnx(model, target, filename='checkpoint.onnx'):
    input = torch.randn(IMAGENET_SIZE)
    filename = target + filename
    if target == 'DPUv1':
        export_dpuv1_onnx(model, input_shape=IMAGENET_SIZE, input_t=input, export_path=filename)
    elif target == 'DPUv2':
        export_dpuv2_onnx(model, input_shape=IMAGENET_SIZE, input_t=input, export_path=filename)
    else:
        raise RuntimeError("Target {} not supported".format(target))
